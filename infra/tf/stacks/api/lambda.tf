...
data "aws_iam_policy_document" "download_lambda" {
  statement {
    ...
    resources = ["${aws_s3_bucket.images.arn}/DownloadImage/*"]
  }
}
...
module "reverse_lambda" {
  ...
  function_name          = local.reverse_lambda_name
  description            = "Downloads status cat image from S3, reverses it, and uploads to S3."
  runtime                = "python3.11"
  lambda_handler         = "lambda_function.lambda_handler"
  memory_size            = 128
  timeout                = 180
  permission_policy_json = data.aws_iam_policy_document.reverse_lambda.json

  lambda_package_bucket_name = local.lambda_package_bucket_name
  lambda_package_object_key  = local.reverse_lambda_package_key

  api_gateway_execution_arn = "${local.apigw_arn_prefix}/*/${aws_api_gateway_method.reverse.http_method}/${aws_api_gateway_resource.reverse.path_part}"
...
data "aws_iam_policy_document" "reverse_lambda" {
  statement {
    sid    = "AllowS3Get"
    effect = "Allow"
  ...
  statement {
    ...
    resources = ["${aws_s3_bucket.image.arn}/${local.reverse_lambda_name}/*"]
  }
}
